...
def auto_annotate(dataset_name: str, video_path: str, model_id: str, labels: List[str], confidence_level: float, overlap_threshold: float, markup_image: bool):
    obj_detect = edgeiq.ObjectDetection(model_id)
    obj_detect.load(engine=edgeiq.Engine.DNN)
    with edgeiq.FileVideoStream(video_path) as video_stream, edgeiq.Streamer(3, inter_msg_time=1) as streamer:
